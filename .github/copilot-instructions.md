...
- 基本的にbiomeにコードのフォーマットは任せる。biomeのルールに従っていれば、コードスタイルについて指摘することはないと思います。
- コンポーネントやその定義ファイルの命名は大文字のキャメルケースで行う。例: `MyComponent`
- 変数や関数の命名は小文字のキャメルケースで行う。例: `myVariable`, `myFunction()`
- コンポーネント以外のファイルの命名は小文字のケバブケースで行う。例: `my-file.tsx`
- 定数の命名は全て大文字のスネークケースで行う。例: `MY_CONSTANT`
- tailwindcssで色を扱う場合は、可能な限りテーマカラーを使用する。テーマカラーにない色を使う必要がある場合は、ui.cssにカスタムクラスを作成して対応する。
- 可能な限り、コードの重複を避けるために関数やコンポーネントを作成して再利用する。
- コメントは必要に応じて適切に記述する。特に、複雑なロジックや、なぜそのコードが必要なのかが明確でない場合は、コメントを追加する。
