...
import type { Input, InputKindToDataType } from "./input";
...
const diaryPanes: Map<Identifier, DiaryPane<unknown>> = new Map();

export function registerDiaryPane<T>(pane: DiaryPane<T>) {
	diaryPanes.set(pane.identifier, pane as DiaryPane<unknown>);
}

export function getDiaryPane<T>(identifier: Identifier) {
	return diaryPanes.get(identifier) as DiaryPane<T> | undefined;
}

export function getAllDiaryPanes() {
	return [...diaryPanes.values()];
}
