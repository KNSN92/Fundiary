...
export async function getUserList(
	limit = 10,
	offset = 0,
): Promise<User[] | ArkErrors> {
	const db = await getDatabase();
	const result = await db.select<unknown[]>(
		"SELECT * FROM Users LIMIT ? OFFSET ?",
		[limit, offset],
	);
	const users = UserValidator.array()(result);
	return users;
}
...
export async function getStreak(id: Uuid): Promise<number | ArkErrors | null> {
	...
	const streakBeginDay = Math.floor(
		streakBeginAt.getTime() / (1000 * 3600 * 24),
	);
	const streakLatestDay = Math.floor(
		streakLatestAt.getTime() / (1000 * 3600 * 24),
	);
...
export async function streakStatus(
	id: Uuid,
): Promise<StreakStatus | ArkErrors | null> {
	const user = await getUser(id);
	if (user == null) {
		return null;
	}
...
export async function updateStreak(
	...
	if (streakLatestDay === today) {
		...
		await db.execute("UPDATE Users SET streakLatestAt = ? WHERE id = ?", [
			date.toISOString(),
			id,
		]);
		return undefined;
